 ...
 */
@Service
public class InvoiceParserTestImpl {
	...
	public static void getSubImage(String _psfn) {
		try {
			BufferedImage bi = ImageIO.read(new FileInputStream(new java.io.File(_psfn)));
			BufferedImage bi_1 = bi.getSubimage(
					Integer.parseInt("2093"),
					(Integer.parseInt("91")	+ (1 * (Integer.parseInt("1") + Integer.parseInt("1")))),
					Integer.parseInt("752"),
					Integer.parseInt("46"));
			...
			String _line_txt =  OCREngineUtils.runTesseract(_inv_part_fn, "");
			System.out.println("LineText: " + _line_txt);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
...
//				}
				...
				if (template != null) {
					...
					if (ocrt != null) {
						...
						for (OCRTemplateMapItem tmi : tmis) {
							if (tmi.getFieldId().startsWith("INVOICE_LINE")) {
							...
							if (tmi.getFieldId().startsWith("INVOICE_LINE_ITEM_DESCRIPTION")) {// if (tmi.getFieldId().equals("inv_line_price")) {// if(tmi.getFieldId().equals("inv_line_total")){
								line_item_tmi = tmi;
							}
						}
